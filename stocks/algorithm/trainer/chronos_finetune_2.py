"""Overnight Chronos-2 LoRA finetuning on the 11700 US stocks split (pretrained base)."""
...
# --- training --------------------------------------------------------------
LEARNING_RATE = 1e-5  # Chronos recommendation for LoRA
FINETUNE_MODE = "lora"
BATCH_SIZE = 16  # counts target+covariate channels → ~8 ticker-pairs / step
NUM_STEPS = 80_000
CONTEXT_LENGTH = 1024
EVAL_SUBSET = 512
EVAL_SEED = 0  # same seed as from-scratch run for comparable eval curves
...
OUTPUT_DIR = RUNS_DIR / f"lora-11700-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
...
def main() -> None:
    ...
    print(
        f"LoRA finetune | steps: {NUM_STEPS} | batch_size: {BATCH_SIZE} | "
    ...
    print(f"Loaded pretrained Chronos-2 with {sum(p.numel() for p in pipeline.model.parameters()):,} parameters")
